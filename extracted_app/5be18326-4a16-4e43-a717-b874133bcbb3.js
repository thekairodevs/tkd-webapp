/* Original blueprint ornaments — modular hardware-y boxes, halftones, and connectors.
   Pure CSS shapes (no third-party SVG). Composable to scatter around the hero & sections. */

const Ornament = ({ x, y, w, h, children, label, corners = false, className = "", style = {} }) => (
  <div
...
const ModDial = ({ x, y, size = 220, label = "MOD-01 / DIAL", parallax = 0 }) => (
  <Ornament x={x} y={y} w={size} h={size} label={label} corners style={{ "--p": parallax }} className="bp-parallax">
...
const ModHalftone = ({ x, y, w = 140, h = 90, label = "FIELD/A", parallax = 0 }) => (
  <Ornament x={x} y={y} w={w} h={h} label={label} className="halftone bp-parallax" style={{ "--p": parallax }}>
    <div style={{
...
const ModStripes = ({ x, y, w = 90, h = 130, label = "STRIPE-D", parallax = 0 }) => (
  <Ornament x={x} y={y} w={w} h={h} label={label} className="stripes-d bp-parallax" style={{ "--p": parallax }}>
...
const ModGrid = ({ x, y, w = 110, h = 110, label = "GRID-MX", parallax = 0 }) => (
  <Ornament x={x} y={y} w={w} h={h} label={label} className="bp-parallax" style={{ "--p": parallax }}>
...
const ModRouter = ({ x, y, w = 200, h = 70, label = "RTR / IO-08", parallax = 0 }) => (
  <Ornament x={x} y={y} w={w} h={h} label={label} className="bp-parallax" style={{ "--p": parallax }}>
...
const ModBars = ({ x, y, w = 80, h = 120, label = "EQ", parallax = 0 }) => (
  <Ornament x={x} y={y} w={w} h={h} label={label} className="bp-parallax" style={{ "--p": parallax }}>
...
const ModDot = ({ x, y, size = 60, ink = false, label, parallax = 0 }) => (
  <div className="bp-parallax" style={{
...
const ModBracket = ({ x, y, w = 160, h = 40, label = "REF/01", parallax = 0 }) => (
  <div className="bp-parallax" style={{ position: "absolute", left: x, top: y, width: w, height: h, "--p": parallax }}>
...
const Connector = ({ x, y, w = 80, h = 1, vertical = false }) => (
  <div style={{
    position: "absolute",
    left: x, top: y,
    width: vertical ? 1 : w,
...
/* —— Hero arrangement: ornaments framing OUTSIDE the card (top band, side rails, bottom band) —— */
const HeroOrnaments = () => (
  <div className="bp-field" aria-hidden="true">
    ...
    <div className="mono bp-parallax" style={{ position: "absolute", left: 760, top: 130, fontSize: 9, letterSpacing: ".18em", color: "var(--muted)", "--p": 0.02 }}>
    ...
    <div className="mono bp-parallax" style={{ position: "absolute", right: 24, top: "82%", fontSize: 9, letterSpacing: ".18em", color: "var(--muted)", "--p": 0.03 }}>
      ED. III
    </div>

    {/* ===== BOTTOM BAND (below card) ===== */}
    ...
    <div className="mono bp-parallax" style={{ position: "absolute", left: "46%", bottom: 56, fontSize: 9, letterSpacing: ".18em", color: "var(--muted)", "--p": 0.02 }}>
      INDEX  /  001 — 042
    </div>
  </div>
);
...
Object.assign(window, {
  Ornament, ModDial, ModHalftone, ModStripes, ModGrid, ModRouter,
  ModBars, ModDot, ModBracket, Connector, HeroOrnaments,
});
